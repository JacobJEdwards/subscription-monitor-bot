...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "Use /check followed by a user id to view the users subscription"
    )
...
async def manualCheck(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        await update.message.reply_text(
            "Please use this command followed by a user id!\nFor example:\n*/check"
            " 6043385959*",
            parse_mode="Markdown",
        )
        return
...
def main() -> None:
    ...
    application.add_handler(
        ChatMemberHandler(memberStatusChange, ChatMemberHandler.CHAT_MEMBER)
    )
